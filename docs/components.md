...
| `/saved-stay` | `SavedStay` | Sign-in and save controls around one stay. | The host supplies account state and persistence; no authentication or `identify()` call lives in the package. |
| `/host-workspace` | `HostWorkspace` | Host-side availability control with visible stable host and team-member IDs. | The host supplies the authenticated IDs, state, and event capture. |
